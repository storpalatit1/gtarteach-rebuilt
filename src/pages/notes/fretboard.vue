...
const emit = defineEmits(['note-pressed'])
...
function playNote(note) {
  ...
  if (note.name.length==2 && isNaN(note.name[1]))
  {
...
<template>
  <div class="relative w-full max-w-4xl">
    ...
    <button
      ...
      :key="index"
      class="absolute rounded-full dark:bg-gray-800 bg-blue-500 opacity-80 hover:opacity-100 transition-transform duration-150 hover:scale-125"
      :style="{
        width: '3%',
        height: '10%',
        top: note.y + '%',
      ...
      @click="playNote(note)"
    >
    <div v-if="props.modifiers!=='blindfold'"> {{note.name }}</div>
   </button>
  </div>
